...
class Game(arcade.View):
    ...
    def draw_horizental(self, grid_size, box_size, pixel):
        ...
        for i in range(1, grid_size):
            arcade.draw_line(0, box_size, (box_size*grid_size),
                             box_size,  arcade.color.BLACK, pixel)
            box_size = box_size + temp

    def draw_vertical(self, grid_size, box_size, pixel):
        temp = box_size
        for i in range(1, grid_size):
            arcade.draw_line(box_size, 0, box_size,
                             (box_size*grid_size),  arcade.color.BLACK, pixel)
            box_size = box_size + temp
    ...
    def on_draw(self):
        ...
        arcade.start_render()
        if self.state == "GameMenu":
            arcade.draw_text("     Welcome to Our Ai-based Snail Game",
                             100, 300, arcade.color.WHITE, font_size=28)
            arcade.draw_text("Press any key to continue", 480, 250,
                             arcade.color.WHITE, font_size=30, anchor_x="center")
            self.__init__()

        elif self.state == "GameOn":
            arcade.draw_lrwh_rectangle_textured(
                0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, back)

            self.draw_horizental(10, boxSize, 4)
            self.draw_vertical(10, boxSize, 4)
            arcade.draw_lrwh_rectangle_textured(
                600, 0, 1000, SCREEN_HEIGHT, back)
            arcade.draw_text("          AI Snail Game", 630, 515,
                             arcade.color.WHITE, font_size=18)
            arcade.draw_text(" Human Score = " +
                             str(self.humanScore), 690, 450, font_size=18)
            arcade.draw_text(" Agent Score = " +
                             str(self.BOTScore), 690, 400, font_size=18)
            arcade.draw_lrwh_rectangle_textured(600, 180, 400, 300, vs)

            arcade.draw_text(" Presented by: ", 690, 240, font_size=18)

            arcade.draw_text(" Nazmul Haque ", 690, 200, font_size=18)
            arcade.draw_text(" Shafin Junayed ", 690, 160, font_size=18)
            arcade.draw_text(" Ashraful Alam ", 690, 120, font_size=18)
            ...
            for row in range(0, len(self.board)):
                ...
                for col in range(0, len(self.board)):
                    if self.board[row][col] == 1:
                        arcade.draw_lrwh_rectangle_textured(
                            X+MARGIN, SCREEN_HEIGHT-Y, boxSize-MARGIN, boxSize-MARGIN, snail1)

                    elif self.board[row][col] == 2:
                        arcade.draw_lrwh_rectangle_textured(
                            X+MARGIN, SCREEN_HEIGHT-Y, boxSize-MARGIN, boxSize-MARGIN, snail2)

                    elif self.board[row][col] == 11:
                        arcade.draw_lrwh_rectangle_textured(
                            X+MARGIN, SCREEN_HEIGHT-Y, boxSize-MARGIN, boxSize-MARGIN, sp1)

                    elif self.board[row][col] == 22:
                        arcade.draw_lrwh_rectangle_textured(
                            X+MARGIN, SCREEN_HEIGHT-Y, boxSize-MARGIN, boxSize-MARGIN, sp2)
            ...
            if self.humanScore > self.BOTScore:
                self.win = "human"
            elif self.humanScore < self.BOTScore:
                self.win = "BOT"
            elif self.humanScore == self.BOTScore:
                self.win = "draw"

            if self.win == "human":
                arcade.draw_text("Congratulations, Human has Won !",
                                 100, 300, arcade.color.WHITE, font_size=35)
                arcade.draw_text("Click to continue", 480, 250,
                                 arcade.color.WHITE, font_size=30, anchor_x="center")

            elif self.win == "BOT":
                arcade.draw_text("             Agent has nailed it :)",
                                 100, 300, arcade.color.WHITE, font_size=35)
                arcade.draw_text("Click to continue", 480, 250,
                                 arcade.color.WHITE, font_size=20, anchor_x="center")

            elif self.win == "draw":
                arcade.draw_text("It's a draw..", 350, 300,
    ...
    def StuckCondition(self):
        if self.Human_Sore_Record == self.humanScore or self.Bot_Score_Record == self.BOTScore:
            self.Score_Checker += 1
        else:
            self.Human_Sore_Record, self.Bot_Score_Record, self.Score_Checker = self.humanScore, self.BOTScore, 0
        ...
        if self.state == "GameOn":
            ...
            if self.checkValidClick(x, y):
                ...
                if self.turn == "human":
                    x, y = self.gethumanPosition()
                    # Right Move
                    if row == x and y+1 == col and self.board[row][col] == 0 or self.board[row][col] == 22 and row == x and y+1 == col:
                    ...
                    # Left Move
                    elif row == x and y-1 == col and self.board[row][col] == 0 or self.board[row][col] == 22 and row == x and y-1 == col:
                        if self.board[row][col] == 22:
                            self.LSlip(1)
                        else:
                            self.board[row][col], self.board[x][y] = 2, 22
                          #  self.turn = "BOT"
                            self.humanScore += 1
                    # down Move
                    elif x+1 == row and y == col and self.board[row][col] == 0 or self.board[row][col] == 22 and x+1 == row and y == col:
                        if self.board[row][col] == 22:
                            self.DownSlip(1)
                        else:
                            self.board[row][col], self.board[x][y] = 2, 22
    ...
    def score(self):
        ...
        if self.humanScore > 49 or self.BOTScore > 49:
            self.state = "GameOver"
            if self.humanScore > self.BOTScore:
                self.win = "human"
            elif self.humanScore < self.BOTScore:
                self.win = "BOT"
        elif self.humanScore == 49 and self.BOTScore == 49:
            self.state = "GameOver"
            self.win = "draw"


if __name__ == "__main__":
    window = arcade.Window(SCREEN_HEIGHT+400, SCREEN_WIDTH, "SNAIL GAME")
    game_view = Game()
